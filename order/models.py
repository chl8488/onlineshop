...
class Order(models.Model):
    ...
    discount = models.IntegerField(default=0, validators=[MinValueValidator(0), MaxValueValidator(100000)])#얼마만큼 할인받앗나

    class Meta:
        ordering = ['-created']# 최신접수된 주문이 위쪽으로올라오게
...
class OrderItem(models.Model):# Order입장에서 OrderItem을 items라 부르겟다
    order = models.ForeignKey(Order, on_delete=models.CASCADE, related_name='items')
    product = models.ForeignKey(Product, on_delete=models.PROTECT, related_name='order_products')
    price = models.DecimalField(max_digits=10, decimal_places=2)# 10자리수 2자리소수점
    quantity = models.PositiveIntegerField(default=1)

    def __str__(self):#파이썬 기본문법으로 object를 프린트할때 무슨내용을 출력할지
        return str(self.id)
...
class OrderTransaction(models.Model):
    ...
    # transaction_id 결제정보를관리하는 회사(iamport)의 기본키(결제취소, 결제정보변경 등 요청)
    transaction_id = models.CharField(max_length=120, null=True, blank=True)
    amount = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    # transaction_status 결제상태를 받아옴
    transaction_status = models.CharField(max_length=220, null=True, blank=True)
    type = models.CharField(max_length=120, blank=True)
    created = models.DateTimeField(auto_now_add=True)
    #success = models.BooleanField(default=False)결제가 실패했습니다 정보를 남기기위해
